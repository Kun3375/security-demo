 ...
 */
@Component
public class SmsCaptchaValidationFilter extends OncePerRequestFilter implements InitializingBean {
    ...
    public void afterPropertiesSet() throws ServletException {
        ...
        String[] configUrls = StringUtils.splitByWholeSeparatorPreserveAllTokens(
                securityProperties.getCaptcha().getStandardCaptcha().getUrls(), ",");
        if (configUrls != null) {
            Collections.addAll(urls, configUrls);
        }
        urls.add(securityProperties.getCommon().getMobileLoginProcessingUrl());
    }
    ...
    private void validate(ServletWebRequest request) throws ServletRequestBindingException {
        // session中存储的验证码正确值
        StandardCaptcha captchaInSession =
                (StandardCaptcha) sessionStrategy.getAttribute(
                        request, AbstractCaptchaProcessor.SESSION_KEY_PREFIX + "SMS");
        // 请求中输入的验证码
        String captchaInRequest = ServletRequestUtils.getStringParameter(request.getRequest(), "smsCaptcha");
        
        if (captchaInSession == null) {
            throw new CaptchaValidationException("验证码不存在");
        }
        
        sessionStrategy.removeAttribute(request, AbstractCaptchaProcessor.SESSION_KEY_PREFIX + "SMS");
